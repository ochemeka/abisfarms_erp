...
// Types that actually deduct stock (real sales)
$stockDeductTypes = ['invoice', 'receipt'];
 
// ── Transaction: create invoice + items + deduct stock ────
$invoice = DB::transaction(function () use (
    $validated, $shopId, $subtotal, $discount,
    $taxRate, $taxAmount, $total, $invoiceNumber,
    $stockDeductTypes
) {
    ...
    $shouldDeductStock = in_array($validated['type'], $stockDeductTypes);
    ...
    foreach ($validated['items'] as $index => $item) {
        ...
        if (!empty($item['product_id'])) {
            // Staff selected from dropdown — direct match
            $product = \App\Models\Product::find($item['product_id']);
        } else {
            // Staff typed manually — try to match by name
            $product = \App\Models\Product::where('shop_id', $shopId)
                ->where('is_active', true)
                ->whereRaw('LOWER(name) = ?', [strtolower(trim($item['description']))])
                ->first();
        }
    ...
    return $invoice; // ← CRITICAL: must return from closure
});
 
// ── Role-based redirect with auto-print ──────────────────
...
$thermalUrl = route("{$prefix}.invoices.thermal", $invoice) . '?autoprint=1';
