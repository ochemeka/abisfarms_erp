    ...
    <div class="py-8">
        <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200
                ...
                <form method="POST"
                      action="{{ route('owner.expenses.update', $expense) }}"
                        ...
                        <div>
                            ...
                            <select name="category"
                                ...
                                @foreach($categories as $key => $label)
                                    <option value="{{ $key }}"
                                            @selected(old('category', $expense->category) === $key)>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div>
                        ...
                        <textarea name="description"
                                  ...
                                  class="w-full text-sm border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2.5
                                         bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-100
                                         focus:outline-none focus:ring-2 focus:ring-bh-red/50 resize-none">{{ old('description', $expense->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                            Receipt <span class="text-gray-400 font-normal">(optional · replaces existing)</span>
                        </label>
                        @if($expense->receipt_path)
                            <p class="text-xs text-gray-500 dark:text-gray-400 mb-2">
                                ...
                                <a href="{{ Storage::url($expense->receipt_path) }}"
                                   target="_blank"
                                   class="text-bh-red hover:underline ml-1">View</a>
                            </p>
                        @endif
                        <input type="file"
                               name="receipt"
