...
<div class="max-w-2xl">
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5">
        <div class="grid grid-cols-2 md:grid-cols-3 gap-4 mb-5 pb-5 border-b border-gray-100 dark:border-gray-700">
            <div>
            ...
            <div>
                ...
                <p class="font-semibold text-gray-800 dark:text-white">₦{{ number_format($tillSession->opening_float, 2) }}</p>
            </div>
            <div>
                <p class="text-xs text-gray-400 mb-1">Expected Cash</p>
                <p class="font-semibold text-gray-800 dark:text-white">₦{{ number_format($tillSession->expected_cash, 2) }}</p>
            </div>
            <div>
                <p class="text-xs text-gray-400 mb-1">Actual Cash</p>
                <p class="font-semibold text-gray-800 dark:text-white">₦{{ number_format($tillSession->actual_cash, 2) }}</p>
            </div>
            <div>
                <p class="text-xs text-gray-400 mb-1">Discrepancy</p>
                <p class="font-semibold {{ ($tillSession->discrepancy ?? 0) != 0 ? 'text-red-500' : 'text-green-600' }}">
        ...
        <form method="POST" action="{{ route('supervisor.till.approve', $tillSession) }}">
            @csrf
            <button type="submit"
