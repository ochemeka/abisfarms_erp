...
<div class="max-w-lg">
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200
        ...
        <div class="bg-gray-50 dark:bg-gray-700/50 rounded-lg px-4 py-3 mb-5 text-sm
                    text-gray-500 dark:text-gray-400">
            ⚠️ Count your cash drawer carefully. Do not look at expected total — this is a blind close.
        </div>

        <form method="POST" action="{{ route('cashier.till.close') }}">
            @csrf
            <div class="mb-4">
            ...
            <div class="mb-5">
                ...
                <textarea name="notes" rows="2" placeholder="Any notes about this session..."
                          class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600
                                 ...
                                 focus:outline-none focus:ring-2 focus:ring-bh-red resize-none">{{ old('notes') }}</textarea>
            </div>
            <button type="submit"
                    class="w-full py-2.5 bg-bh-red hover:bg-bh-dark text-white
                           font-medium rounded-lg transition-colors"
                    onclick="return confirm('Close this till session? This cannot be undone.')">
                Close Till Session
            </button>
        </form>
    </div>
