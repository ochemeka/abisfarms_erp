...
<div class="bg-white dark:bg-gray-800 rounded-xl border border-green-300 dark:border-green-700
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-5">
        ...
        <div>
            ...
            <p class="text-xl font-bold text-gray-800 dark:text-white">
                {{ $session->sales->where('status','completed')->count() }}
            </p>
        </div>
        <div>
    ...
    <a href="{{ route('supervisor.till.close.form') }}"
...
<div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700
    ...
    <form method="POST" action="{{ route('supervisor.till.open') }}">
        @csrf
        <div class="mb-4">
...
<div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
    <div class="px-5 py-4 border-b border-gray-100 dark:border-gray-700">
        <h3 class="text-sm font-semibold text-gray-800 dark:text-white">Today's Sessions</h3>
    </div>
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                    ...
                    <td class="px-5 py-3 text-xs text-gray-500">{{ $s->opened_at->format('h:i A') }}</td>
                    <td class="px-5 py-3 font-medium text-gray-800 dark:text-white">
                        ₦{{ number_format($s->opening_float, 2) }}
                    </td>
                    <td class="px-5 py-3">
                        @php $badge = match($s->status) { 'open' => 'bg-green-100 text-green-700', 'closed' => 'bg-gray-100 text-gray-600', 'flagged' => 'bg-red-100 text-red-700', default => 'bg-gray-100 text-gray-500' }; @endphp
                        <span class="px-2 py-1 rounded-full text-xs font-medium capitalize {{ $badge }}">{{ $s->status }}</span>
                    </td>
                    <td class="px-5 py-3 text-xs {{ ($s->discrepancy ?? 0) != 0 ? 'text-red-500 font-semibold' : 'text-gray-400' }}">
                        @if($s->discrepancy) {{ $s->discrepancy > 0 ? '+' : '' }}₦{{ number_format($s->discrepancy, 2) }} @else — @endif
                    </td>
                    <td class="px-5 py-3">
                        @if($s->status === 'flagged')
                        <a href="{{ route('supervisor.till.reconcile', $s) }}"
